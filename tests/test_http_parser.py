...
def test_bad_chunked(parser: HttpRequestParserPy) -> None:
    ...
    with pytest.raises(
        (http_exceptions.BadHttpMessage, http_exceptions.TransferEncodingError),
        match="0_2e",
    ):
        parser.feed_data(text)
...
async def test_request_chunked_with_trailer(parser: Any) -> None:
    text = (
        b"GET /test HTTP/1.1\r\n"
        b"Transfer-Encoding: chunked\r\n\r\n"
        b"4\r\ntest\r\n"
...
async def test_request_chunked_reject_bad_trailer(parser: Any) -> None:
    text = (
        b"GET /test HTTP/1.1\r\n"
        b"Transfer-Encoding: chunked\r\n\r\n"
        b"0\r\n"
    ...
    with pytest.raises(http_exceptions.BadHttpMessage, match=r"Invalid HTTP Header|bad.*trailer"):
        parser.feed_data(text)
